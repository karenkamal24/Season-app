...
class GeographicalGuideService
{
    ...
    public function update(Request $request, $id): GeographicalGuide
    {
        ...
        // Get data directly from request (form-data)
        // Read all input and filter only allowed fields
        ...
        foreach ($allowedFields as $field) {
            if (array_key_exists($field, $allInput) && $allInput[$field] !== null && $allInput[$field] !== '') {
                $data[$field] = $allInput[$field];
            }
        }

        // Validate the data if there's any
        if (!empty($data)) {
            $requestInstance = new \App\Http\Requests\GeographicalGuide\UpdateGeographicalGuideRequest();
