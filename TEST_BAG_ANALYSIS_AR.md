# 🤖 اختبار تحليل الحقيبة بالذكاء الاصطناعي

## 🎯 Endpoint

```
POST /api/smart-bags/{bagId}/analyze
```

---

## ✅ المتطلبات الأساسية

### 1. يجب أن يكون عندك:
- ✅ حقيبة موجودة (`bag_id`)
- ✅ أغراض في الحقيبة (على الأقل غرض واحد)
- ✅ Token صحيح

### 2. الحقيبة **لا تكون فارغة**:
```json
❌ إذا الحقيبة فارغة:
{
  "success": false,
  "message": "Cannot analyze empty bag. Please add items first.",
  "message_ar": "لا يمكن تحليل حقيبة فارغة. الرجاء إضافة أغراض أولاً."
}
```

---

## 🚀 كيف تجرب في Postman؟

### الخطوة 1: تأكد من وجود حقيبة مع أغراض

```http
GET /api/smart-bags
Authorization: Bearer YOUR_TOKEN
Accept-Language: ar
```

احفظ الـ `bag_id` من الـ response.

### الخطوة 2: قم بالتحليل

```http
POST /api/smart-bags/1/analyze
Authorization: Bearer YOUR_TOKEN
Accept-Language: ar
Content-Type: application/json

{}
```

**أو مع preferences (اختياري):**

```json
{
  "preferences": [
    "أدوية",
    "مستندات مهمة"
  ]
}
```

---

## 📋 Request Details

### Headers:
```
Authorization: Bearer YOUR_TOKEN
Accept-Language: ar (للعربية) أو en (للإنجليزية)
Content-Type: application/json
```

### Body (اختياري):
```json
{
  "preferences": ["أدوية", "مستندات"],
  "force_reanalysis": false
}
```

### Parameters:

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `preferences` | array | No | تفضيلات إضافية للتحليل |
| `force_reanalysis` | boolean | No | إعادة التحليل حتى لو تم مؤخراً |

---

## ✅ Response في حالة النجاح

```json
{
  "success": true,
  "message": "تم تحليل الحقيبة بنجاح",
  "data": {
    "id": 1,
    "bag_id": 1,
    "missing_items": [
      {
        "item": "شاحن لابتوب",
        "category": "إلكترونيات",
        "reason": "ضروري للابتوب",
        "priority": "high"
      },
      {
        "item": "أدوية شخصية",
        "category": "أدوية وعناية",
        "reason": "مهم للرحلات الطويلة",
        "priority": "medium"
      }
    ],
    "unnecessary_items": [
      {
        "item": "كتب ثقيلة",
        "reason": "يمكن استخدام نسخة إلكترونية",
        "weight_saved": 2.5
      }
    ],
    "weight_optimization": {
      "current_weight": 15.3,
      "optimal_weight": 12.8,
      "can_reduce": 2.5,
      "suggestions": [
        "استبدل الكتب بنسخ إلكترونية",
        "استخدم منتجات مصغرة للعناية"
      ]
    },
    "smart_alerts": [
      {
        "type": "medicines_missing",
        "severity": "high",
        "message": "لا توجد أدوية في الحقيبة",
        "action": "أضف الأدوية الأساسية"
      },
      {
        "type": "documents_missing",
        "severity": "medium",
        "message": "لا توجد مستندات عمل",
        "action": "راجع المستندات المطلوبة"
      }
    ],
    "additional_suggestions": [
      "احزم ملابس إضافية ليوم واحد",
      "لا تنسى شاحن محمول",
      "تأكد من جواز السفر"
    ],
    "confidence_score": 0.92,
    "processing_time_ms": 1250,
    "analyzed_at": "2026-01-01T22:30:45+00:00"
  }
}
```

---

## ⚠️ حالات الخطأ

### 1. الحقيبة فارغة:
```json
{
  "success": false,
  "message": "لا يمكن تحليل حقيبة فارغة. الرجاء إضافة أغراض أولاً."
}
```
**الحل:** أضف أغراض للحقيبة أولاً.

### 2. تم التحليل مؤخراً (أقل من 24 ساعة):
```json
{
  "success": false,
  "message": "تم تحليل الحقيبة مؤخراً. استخدم force_reanalysis=true لإعادة التحليل.",
  "last_analyzed_at": "2026-01-01T21:00:00+00:00"
}
```
**الحل:** أضف `"force_reanalysis": true` في Body.

### 3. خطأ عام:
```json
{
  "success": false,
  "message": "فشل في تحليل الحقيبة",
  "error": "..."
}
```

---

## 🎬 سيناريو كامل في Postman

### 1️⃣ أنشئ حقيبة
```
POST /api/smart-bags
Body:
{
  "name": "رحلة دبي",
  "trip_type": "عمل",
  "duration": 4,
  "destination": "دبي",
  "departure_date": "2026-02-01",
  "max_weight": 20
}
```
احفظ الـ `bag_id` (مثلاً: 1)

### 2️⃣ أضف أغراض
```
POST /api/smart-bags/1/items
Body:
{
  "name": "لابتوب",
  "weight": 2.3,
  "item_category_id": 3
}
```

```
POST /api/smart-bags/1/items
Body:
{
  "name": "ملابس رسمية",
  "weight": 3.5,
  "item_category_id": 1,
  "quantity": 3
}
```

### 3️⃣ احصل على تفاصيل الحقيبة
```
GET /api/smart-bags/1
```
تأكد من وجود الأغراض.

### 4️⃣ قم بالتحليل! 🚀
```
POST /api/smart-bags/1/analyze
Body: {}
```

### 5️⃣ شاهد النتيجة! 🎉

---

## 🔄 إعادة التحليل (Force Reanalysis)

إذا غيّرت الأغراض وعايز تحلل تاني قبل مرور 24 ساعة:

```json
{
  "force_reanalysis": true
}
```

---

## 📊 endpoints إضافية

### احصل على آخر تحليل:
```
GET /api/smart-bags/{bagId}/analysis/latest
```

### احصل على تاريخ التحليلات:
```
GET /api/smart-bags/{bagId}/analysis/history
```

### احصل على التنبيهات الذكية:
```
GET /api/smart-bags/{bagId}/smart-alert
```

---

## 💡 نصائح مهمة

1. ✅ **أضف أغراض كافية** - كلما زادت الأغراض، كان التحليل أفضل
2. ✅ **حدد نوع الرحلة بدقة** - التحليل يعتمد عليه
3. ✅ **أضف التفضيلات** - مثل "أدوية" أو "مستندات"
4. ⚠️ **انتبه للوزن** - التحليل يحذرك إذا قاربت الحد الأقصى

---

## 🎯 في Postman

موجود في:
```
🤖 AI Features → Analyze Bag - تحليل ذكي للحقيبة
```

أو في:
```
📊 Complete Test Scenario → 6️⃣ Analyze with AI
```

---

## ✨ جاهز للتجربة!

الآن جرّب التحليل وشوف الاقتراحات الذكية! 🚀

**ملاحظة:** التحليل يستغرق ثوانٍ قليلة حسب عدد الأغراض.

